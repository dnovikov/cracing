...
Game::setup (unsigned int seed)
{
  ...
  if (!(this->track_win
        = newwin (this->max_row, this->max_col - INFO_WIN_WIDTH - 2, 0,
                  INFO_WIN_WIDTH)))
    {
  ...
  this->vehicle = new Vehicle (((this->max_col - INFO_WIN_WIDTH) / 2),
                               this->max_row - 1, 3);
  ...
  do
    {
      ...
      this->info->show_info (this->info_win, this->vehicle->get_speed (),
                             this->vehicle->get_distance (),
                             this->direction_offset, this->turns);
