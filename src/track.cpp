...
void
Track::calculate_new_offsets ()
{
  int new_x_offset = this->x_offset, new_width = this->width, offset_delta,
      width_delta;
  ...
  while (abs (offset_delta) > 1 || (abs (width_delta) > 1)
         || (abs (offset_delta) == 1 && abs (width_delta) == 1)
         || new_width > 7 || new_width < 2);
  ...
  else
    {
      if (this->x_offset <= 0)
      ...
      else
        {
          this->preserve_x_offset_lines
              = rand ()
                % ((this->max_col - this->x_buf_offset - this->width) / 2);
        }
    }
}

void
Track::assign_vehicle (Vehicle *vehicle)
{
  this->vehicle = vehicle;
}

void
Track::shift_y (int lines_num)
{
  int i, j;
